              ...
              <CardContent className="space-y-6">
                <div className="flex items-center gap-6">
                  <Avatar className="h-20 w-20 border border-border">
                    ...
                    <AvatarFallback className="text-xl bg-primary/10 text-primary">EA</AvatarFallback>
                  </Avatar>
                ...
                <div className="grid gap-4 md:grid-cols-2">
                  ...
                  <div className="space-y-2">
                    ...
                    <Input id="company" value={user.company} onChange={e => setUser({...user, company: e.target.value})} required />
                  </div>
                  <div className="space-y-2">
        ...
        <TabsContent value="billing" className="space-y-6 animate-in fade-in-50 duration-500">
          <Card>
             <CardHeader>
               ...
               <CardDescription>Estás utilizando el plan <strong>{user.plan}</strong>.</CardDescription>
             </CardHeader>
             <CardContent className="space-y-8">
               ...
               <div className="space-y-3">
                 ...
                 <p className="text-xs text-muted-foreground">Te estás acercando al límite de tu plan Starter. Mejora tu plan para ejecuciones ilimitadas.</p>
               </div>
             </CardContent>
...
function IntegrationCard({ name, description, connected }: { name: string, description: string, connected: boolean }) {
  return (
    <div className="p-5 border rounded-xl flex flex-col justify-between items-start space-y-4 bg-card/50 hover:bg-muted/10 transition-colors">
      ...
      <Button variant={connected ? "outline" : "secondary"} size="sm" className="w-full" onClick={() => !connected && toast.info(`Redirigiendo a OAuth de ${name}`)}>
