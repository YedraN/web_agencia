...
export default function FacturasPage() {
  ...
  return (
    <div className="flex-1 space-y-6">
      <div>
        <h2 className="text-3xl font-bold tracking-tight text-white">Facturas</h2>
        <p className="text-white/40 text-sm mt-1">
          Historial de facturación y documentos descargables.
        </p>
      </div>
      ...
      <div className="flex gap-6 items-start">
        ...
        <aside className="w-52 shrink-0 space-y-5">
          <div className="rounded-xl border border-white/[0.06] bg-white/[0.02] p-4 space-y-4">
            <p className="text-xs font-semibold text-white/50 uppercase tracking-wider">Filtros</p>

            {/* Búsqueda */}
            <div className="space-y-1.5">
              <label className="text-xs text-white/50">Número de factura</label>
              <div className="flex gap-1.5">
                <Input
                  ...
                  className="bg-white/[0.04] border-white/[0.08] text-white placeholder:text-white/30 text-sm"
                />
                <Button
                  size="icon"
                  variant="outline"
                  onClick={applySearch}
                  className="shrink-0 border-white/[0.08] text-white/60 hover:text-white"
                >
            ...
            <Table>
              <TableHeader>
                <TableRow className="border-white/[0.06] hover:bg-transparent">
                  <TableHead className="text-white/50 font-medium text-xs uppercase tracking-wider pl-4">
                    #
                  </TableHead>
                  ...
                  <TableHead className="text-white/50 font-medium text-xs uppercase tracking-wider text-right">
                    Monto
                  </TableHead>
                  <TableHead className="text-white/50 font-medium text-xs uppercase tracking-wider">
                    Estado
                  </TableHead>
                  <TableHead className="text-white/50 font-medium text-xs uppercase tracking-wider text-right pr-4">
                    Acciones
                  </TableHead>
                </TableRow>
              </TableHeader>
              ...
              <TableBody>
                {isLoading ? (
                  ...
                  <TableRow>
                    <TableCell colSpan={7} className="py-16">
                      <div className="flex flex-col items-center gap-3 text-white/30">
                        ...
                        <p className="text-sm">No hay facturas para los filtros seleccionados.</p>
                      </div>
                    </TableCell>
                  </TableRow>
                ) : (
                  data.items.map((invoice) => (
                    ...
                    >
                      <TableCell className="pl-4 font-mono text-sm text-white/80">
                        {invoice.numero}
                      </TableCell>
                      <TableCell className="text-white/60 text-sm">
                        {formatDate(invoice.emitida_en || invoice.creado)}
                      </TableCell>
                      ...
                      <TableCell className="text-right font-semibold text-white text-sm">
                        {formatAmount(invoice.total_cents, invoice.moneda)}
                      </TableCell>
                      <TableCell>
          ...
          {data && data.total_pages > 1 && (
            <div className="flex items-center justify-between px-1">
              <p className="text-xs text-white/40">
                {data.total} factura{data.total !== 1 ? "s" : ""} en total
              </p>
              <div className="flex items-center gap-1">
                ...
                {Array.from({ length: data.total_pages }, (_, i) => i + 1)
                  .filter((p) => {
                    ...
                    return p === 1 || p === data.total_pages || Math.abs(p - cur) <= 1;
                  })
                  ...
                  .map((p, i) =>
                    p === "..." ? (
                      <span key={`ellipsis-${i}`} className="text-white/30 text-sm px-1">
                        …
                      </span>
                    ) : (
                      <Button
